...
import EntityGivenReviewsDashboard from './EntityGivenReviewsDashboard';

export default function EntityReviewsDashboard({ entityId }) {
    const [reviews, setReviews] = useState([]);
    const [statistics, setStatistics] = useState({});
    const [loading, setLoading] = useState(true);
    const [selectedMainTab, setSelectedMainTab] = useState('received'); // New main tab state
    ...
    useEffect(() => {
        if (entityId && selectedMainTab === 'received') {
            fetchReviews();
        }
    }, [entityId, selectedTab, selectedMainTab]);
    ...
    const handleReply = async (e) => {
        ...
        try {
            ...
            fetchReviews(); // Refresh to show updated reply count
        } catch (error) {
            toast.error('Failed to post reply');
        }
    };

    const handleDispute = async (review) => {
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-lg rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700">
                <div className="border-b border-gray-200 dark:border-gray-700">
                    <nav className="flex space-x-8 px-8 pt-6">
                        <button
                            onClick={() => setSelectedMainTab('received')}
                            className={`py-3 px-1 border-b-2 font-semibold text-lg transition-all ${selectedMainTab === 'received'
                        ...
                        <button
                            onClick={() => setSelectedMainTab('given')}
                            className={`py-3 px-1 border-b-2 font-semibold text-lg transition-all ${selectedMainTab === 'given'
                                ? 'border-green-500 dark:border-green-400 text-green-600 dark:text-green-200'
                                : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-green-600 dark:hover:text-green-200 hover:border-gray-300 dark:hover:border-gray-600'
                                }`}
                        >
                            Reviews Given
                        </button>
            ...
            {selectedMainTab === 'received' ? (
                <>
                    {/* Statistics for received reviews */}
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-lg p-8 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700">
                            ...
                            <div className="text-base text-gray-600 dark:text-gray-300">Average Rating Received</div>
                        ...
                        <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-lg p-8 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700">
                            ...
                            <div className="text-base text-gray-600 dark:text-gray-300">Total Reviews Received</div>
                        </div>
                        <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-lg p-8 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700">
                                    ...
                                    <div>
                                        ...
                                        <textarea
                                            ...
                                            placeholder={replyData.reply_type === 'dispute' ?
                                                ...
                                                "Write your response to this review...(Min. 10 characters required)"
                                            }
                                        />
                                    </div>
                ...
                <EntityGivenReviewsDashboard entityId={entityId} />
            )}
        </div>
    );
}
